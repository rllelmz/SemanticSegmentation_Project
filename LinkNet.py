...
for i in range(len(x_train)):
    x_train[i] = np.resize(x_train[i], (512, 256, 3))
    
for j in range(len(y_train)):
    y_train[j] = np.resize(y_train[j], (512, 256, 3))

for k in range(len(x_test)):
    x_test[k] = np.resize(x_test[k], (512, 256, 3))

for l in range(len(y_test)):
    y_test[l] = np.resize(y_test[l], (512, 256, 3))
...
# Define model
model = sm.Linknet(BACKBONE, input_shape=(512, 256, 3), classes=3, activation='softmax')
model.compile(
    'Adam',
